...
result_loc = "paper2/Results/ML/Financials/hedge_portfolio/hedge_portfolio.pkl"
...
text_transformer_path = load_sentiment_loc(results_dict)
...
text_features_transformer_stock = textual_data[fintextsim_version]["text_features_stock"]

#text_features_llm_fin = pd.read_csv(text_llm_fin_path)
#text_features_llm_stock = pd.read_csv(text_llm_stock_path)
...
df_names = ["text_only", "fin", "fin_mean_sentiment", "fin_text_transformer"]
...
for shift_year in shift_years:
    ...
    return_base = hp.run_cumulative_return_base(
        ...
        model_name = "lr", #name of the ML model for which we want to load the results --> lr, rf or xgb (potentially with _detrend) --> not relevant in this case as we only create the base and update it in the iteration over models
