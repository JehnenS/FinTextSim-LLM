...
result_file = "paper2/Data/Features/text_features/sentiment_rag_fd_sp500_quantile_temporal.pkl" 
...
knn_loc = "paper2/Results/knn/knn_centroid_fd_sp500_quantile_temporal.pkl"
...
rag_answers = prep.load_rag_results("paper2/Results/RAG/rag_temporal_out.jsonl")

results = {}
from feature_creation.TextFeatureCreatorTransformer import TextFeatureCreatorTransformer
from rag.utils_rag import transform_rag_output
...
for model_name in knn_data.keys():
    ...
    print(f"Number of answers: {len(rag_answers)}")
    indices, noise = transform_rag_output(indices, rag_answers, topic_names)
    
    #df_grouped = prep.build_grouped_df(meta_merged, texts, indices, topics)
    
    
    
    creator = TextFeatureCreatorTransformer(sentiment_assignments, indices, meta, cik_to_ticker)
